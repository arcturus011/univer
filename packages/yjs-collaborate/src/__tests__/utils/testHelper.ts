...
export const encV1 = {
  ...
  updateEventName: /** @type {'update'} */ ('update') as 'update',
...
export const encV2 = {
  ...
  updateEventName: /** @type {'updateV2'} */ ('updateV2')  as 'updateV2',
...
export class TestConnector {
   ...
   * @param {prng.PRNG} gen
   */
  constructor (gen = prng.create(random.uint32())) {
    /**
...
export const compare = users => {
  ...
  ymapkeys.forEach(key => t.assert(object.hasProperty(userMapValues[0], key as string)))
  /**
   * @type {Object<string,any>}
   */
  const mapRes = {}
...
export const applyRandomTests = (tc, mods, iterations, initTestObject) => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const test = prng.oneOf(gen, mods) as any
